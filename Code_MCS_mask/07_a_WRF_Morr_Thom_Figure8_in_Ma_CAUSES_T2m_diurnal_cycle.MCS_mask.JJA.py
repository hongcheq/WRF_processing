...
T2_May_yes_MCS = T2_regrid_yes_MCS[:738,:,:]
T2_JJA_yes_MCS = T2_regrid_yes_MCS[738:,:,:]

T2_May_no_MCS = T2_regrid_no_MCS[:738,:,:]
T2_JJA_no_MCS = T2_regrid_no_MCS[738:,:,:]

T2_WRF_May_yes_MCS = T2_May_yes_MCS.sel(lat=slice(lat_1, lat_2), lon=slice(lon_1, lon_2)).mean(dim='lat').mean(dim='lon')
T2_WRF_JJA_yes_MCS = T2_JJA_yes_MCS.sel(lat=slice(lat_1, lat_2), lon=slice(lon_1, lon_2)).mean(dim='lat').mean(dim='lon')

T2_WRF_May_no_MCS = T2_May_no_MCS.sel(lat=slice(lat_1, lat_2), lon=slice(lon_1, lon_2)).mean(dim='lat').mean(dim='lon')
T2_WRF_JJA_no_MCS = T2_JJA_no_MCS.sel(lat=slice(lat_1, lat_2), lon=slice(lon_1, lon_2)).mean(dim='lat').mean(dim='lon')

### Lessons learned: the groupby('time.hour') command requires the time has regular values, just check
print(T2_WRF_May_yes_MCS.coords['time'][0:24])
print(T2_WRF_JJA_yes_MCS.coords['time'][0:24])

### the time has regular values, so you can use the following command
WRF_May_yes_MCS = T2_WRF_May_yes_MCS.groupby('time.hour').mean()
WRF_JJA_yes_MCS = T2_WRF_JJA_yes_MCS.groupby('time.hour').mean()

WRF_May_no_MCS = T2_WRF_May_no_MCS.groupby('time.hour').mean()
WRF_JJA_no_MCS = T2_WRF_JJA_no_MCS.groupby('time.hour').mean()
...
### Plot ###
x_axis = WRF_May_yes_MCS.coords['hour']
...
ax1.plot(x_axis, WRF_JJA_yes_MCS, 'b--', label='WRF_Morri_yes_MCS, JJA')
ax1.plot(x_axis, WRF_JJA_no_MCS, 'y--', label='WRF_Morri_no_MCS, JJA')
...
fig.savefig("../Figure_MCS_mask/T2m.WRF_vs_ARM_SGP"+label_string+".MCS_masks.png",dpi=600,bbox_inches="tight")
plt.show()

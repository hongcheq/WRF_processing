...
## Figure 5 in Feng et al 2018, average over -103, -87
lat_1 = 32.0   # lat = (32N, 48N) consistent with Figure 8 four subregions.
lat_2 = 48.0   # But Feng paper does not say lat boundary
lon_1 = -103.0
lon_2 = -87.0
...
### In Feng et al [2018], Figure 7, MCS precip is adjusted by * (MCS hours / total hours) ?
###                                 non-MCS precip is adjusted by * (non-MCS hours / total hours )?
var_regrid_yes_MCS_obs = var_regrid_yes_MCS_obs * mcspcpct_OBS / ntimes_OBS[0]
var_regrid_no_MCS_obs = var_regrid_no_MCS_obs * (ntimes_OBS[0] - mcspcpct_OBS) / ntimes_OBS[0]
...
ax1.text(s='Precip, WRF vs OBS_NMQ', x=0, y=1.02, ha='left', va='bottom', \
        fontsize=fontsize, transform=ax1.transAxes)
ax1.plot(x_axis, OBS_yes_MCS.values, 'k-', label='OBS_yes_MCS')
ax1.plot(x_axis, OBS_no_MCS.values, 'k--', label='OBS_no_MCS')
...
ax1.set(xlabel='UTC(LST+6hr)', ylabel='Precip, mm/day', title='Precip, 2011 May-Aug')
...
fig.savefig("../Figure_MCS_mask/07_c_Precip.OBS_vs_WRF_Morri"+label_string+".MCS_masks.png",dpi=600,bbox_inches="tight")
plt.show()

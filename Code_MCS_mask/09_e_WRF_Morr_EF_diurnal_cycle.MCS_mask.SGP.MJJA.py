...
ds_WRF_LH = xr.open_dataset('/home/qin5/Data/WRF_postprocessing/WRF_Morri/WRF.postprocessing.extract.hourly.0.04.LH.time_revise.nc')
ds_WRF_HFX = xr.open_dataset('/home/qin5/Data/WRF_postprocessing/WRF_Morri/WRF.postprocessing.extract.hourly.0.04.HFX.time_revise.nc')
...
ds_ARMBE2D_LH = xr.open_dataset('/home/qin5/Data/ARMBE2DGRID/OBS_MCS_mask_consistency/sgparmbe2dgridX1.c1.201105_08.time_revise.latent_heat_flux.nc')
ds_ARMBE2D_SH = xr.open_dataset('/home/qin5/Data/ARMBE2DGRID/OBS_MCS_mask_consistency/sgparmbe2dgridX1.c1.201105_08.time_revise.sensible_heat_flux.nc')
...
LH_regrid = ds_WRF_LH['LH_regrid']
HFX_regrid = ds_WRF_HFX['HFX_regrid']
sum_LH_HFX_regrid = LH_regrid + HFX_regrid
#EF_regrid = np.divide(LH_regrid.values,sum_LH_HFX_regrid,out=np.zeros_like(LH_regrid),where= sum_LH_HFX_regrid!=0)
EF_regrid = np.divide(LH_regrid.values,sum_LH_HFX_regrid,out=np.zeros_like(LH_regrid),where= sum_LH_HFX_regrid>90.0)

###
var_regrid_yes_MCS = EF_regrid.where(WRF_MCS_mask > 0)
var_regrid_no_MCS = EF_regrid.where(np.isnan(WRF_MCS_mask))
...
ARMBE_lh = -ds_ARMBE2D_LH['latent_heat_flux']
ARMBE_sh = -ds_ARMBE2D_SH['sensible_heat_flux']

sum_lh_sh = ARMBE_lh + ARMBE_sh
#ARMBE_var = np.divide(ARMBE_lh.values,sum_lh_sh,out=np.zeros_like(ARMBE_lh),where= sum_lh_sh!=0)
ARMBE_var = np.divide(ARMBE_lh.values,sum_lh_sh,out=np.zeros_like(ARMBE_lh),where= sum_lh_sh > 90.0)

str_var_plt = "EF"
str_var_plt_units = "unitless"
...
fig.savefig("../Figure_MCS_mask/09_e_"+str_var_plt+".OBS_vs_WRF_Morri"+label_string+".MCS_masks.png",dpi=600,bbox_inches="tight")
plt.show()

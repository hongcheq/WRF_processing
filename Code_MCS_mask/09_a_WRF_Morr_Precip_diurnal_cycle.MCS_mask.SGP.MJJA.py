...
ds_WRF = xr.open_dataset('/home/qin5/Data/WRF_postprocessing/WRF_Morri/WRF.postprocessing.extract.hourly.0.04.RAIN_tot.time_revise.nc')
...
ds_ARMBE2D_pr = xr.open_dataset('/home/qin5/Data/ARMBE2DGRID/OBS_MCS_mask_consistency/sgparmbe2dgridX1.c1.201105_08.time_revise.precip_rate.nc')
...
var_regrid_yes_MCS = ds_WRF['RAIN_tot_regrid'].where(WRF_MCS_mask > 0)
var_regrid_no_MCS = ds_WRF['RAIN_tot_regrid'].where(np.isnan(WRF_MCS_mask))
...
ARMBE_pr = ds_ARMBE2D_pr['precip_rate']
...
#lee_way = 0.20                    # then total total SGP box will be regarded as MCS present.
...
pr_ARMBE_SGP_mean =  ARMBE_pr.sel(lat=slice(lat_1, lat_2), lon=slice(lon_1, lon_2)).mean(dim='lat').mean(dim='lon')
...
pr_ARMBE_SGP_mean.coords['time'] = mcspcpmask_OBS_SGP_MCS_points.coords['time']
print(pr_ARMBE_SGP_mean.coords['time'])
print(mcspcpmask_OBS_SGP_MCS_points.coords['time'])

var_regrid_yes_MCS_obs = pr_ARMBE_SGP_mean.where(mcspcpmask_OBS_SGP_MCS_points > threshold)
var_regrid_no_MCS_obs = pr_ARMBE_SGP_mean.where(mcspcpmask_OBS_SGP_MCS_points <= threshold)
...
ax1.text(s='leeway_num=0.1', x=1.0, y=1.02, ha='right', va='bottom', \
        fontsize=fontsize, transform=ax1.transAxes)
...
ax1.set(xlabel='UTC(LST+6hr)', ylabel='Precip, mm/day', title='Precip, ARMBE2D, 2011 May-Aug')
